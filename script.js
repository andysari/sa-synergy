document.addEventListener("DOMContentLoaded", () => {
  ...
  const testimonies = [
    ...
    { nom:"Marina", role:"Entrepreneur", texte:"Andy and I collaborated in the development of my business in Athens. He really helped me and I always consider his advices.\n8 years later I still ask him for his opinion :)\nMuch recommended professional and a very positive human being.", image:"assets/marina.jpg" },
    ...
    { nom:"Alexandre", role:"Assistant comptable", texte:"Très pédagogue et sérieux.\nC'est un professeur très polyvalent qui s'intéresse à énormément de domaines tout en en étant passionné. Très curieux et se posant beaucoup de question, il a un sens critique affûté. Pour finir, il possède beaucoup de compétences et de connaissance à transmettre, cela est symbolisé par son nombre de certifications et de formations passées.", image:"assets/alexandre.jpg" },
  ...
  function renderTestimony(i) {
    ...
    // sanitize line breaks for HTML
    const safeText = (t.texte || "").replace(/\n/g, "<br>");

    display.innerHTML = `
      <img src="${t.image}" alt="${t.nom}" class="temoignage-img" onerror="this.style.display='none'">
      <h3 class="temoignage-name">${t.nom}</h3>
      <h4 class="temoignage-role">${t.role}</h4>
      <p class="temoignage-text">${safeText}</p>
      <div class="temoignage-footer">${t.nom}, ${t.role}</div>
    `;

    // Ensure text is readable and layout stable (no jump) — force minimal height if needed
    // (keeps card height consistent between items)
    display.querySelectorAll('.temoignage-text').forEach(p => p.style.minHeight = '120px');
  }
  ...
  if (display) {
    ...
    // small convenience: keyboard arrows
